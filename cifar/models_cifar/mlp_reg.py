...
FC_WEIGHT_DECAY= 0.0000005
#da
#FC = 0.00001 __last test = 0.00005
#sans da
#FC = 0.00001 __last test = 0.00001
...
def inference(inputs, training_mode):
    ...
    with tf.variable_scope('layer_3'):
        ...
        x = fc_e(x, n_out)
        x = activation(x)
        #x = tf.cond(training_mode, lambda: tf.nn.dropout(x, 0.15), lambda: tf.nn.dropout(x, 1))

    with tf.variable_scope('layer_2'):
        n_out = 512
        x = fc_e(x, n_out)
        x = activation(x)
        #x = tf.cond(training_mode, lambda: tf.nn.dropout(x, 0.25), lambda: tf.nn.dropout(x, 1))
