...
def init_config():
    ...
    #model config
    parser.add_argument('-mp',       '--model_path'       , default='mnist.models_mnist',                         help='model folder, must contain __init__ file')
    parser.add_argument('-m',        '--model'            , default='baseline_reg',                                   help='model file')
    #data config
    parser.add_argument('-dp',       '--dataset'          , default='mnist.dataset_mnist',                        help='folder with metadata configuration')
    parser.add_argument('-d',        '--data'             , default='./cifar/dataset_mnist/bin',                  help='folder with data')
    #saves and logs folders
    parser.add_argument('-ca',       '--cache'            , default='/net/phoenix/blot/mnist/expe',               help='folder with experiments log and variables')
    parser.add_argument('-log',      '--log'              , default='./mnist/expe',                               help='folder with experiments log and variables')
    ...
    #training config
    parser.add_argument('-ne',       '--n_epoch'          , default=300,                                          help='total number of epoch')
    parser.add_argument('-bs',       '--batch_size'       , default=100,                                          help='number of exemple per batch')
    #parser.add_argument('-chkp',     '--checkpoint'       , default=50,                                          help='number of batch for each checkpoint')
    parser.add_argument('-ntrain',   '--n_data_train'     , default=60000,                                        help='number of data in train set')
